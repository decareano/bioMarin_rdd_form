...
function processAllFieldsOfTheForm(req, res) {
    ...
    form.parse(req, function (err, fields, files) {
    	...
    	if (files.profilepic.name) {
            ...
            var dest = fs.createWriteStream('./screenshots/' + files.profilepic.name);
            source.pipe(dest);

    	}
    	
		fs.readFile("database.json", function(err, databaseString){       //refactor into readDatabase 
            var database = JSON.parse(databaseString)
            database.push({fields:fields, files:files})
            fs.writeFile("database.json", JSON.stringify(database, null, 2), function(err){   //refactor into write db method
            	res.writeHead(302, {
